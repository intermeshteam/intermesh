...
export default function AgentsDirectoryPage() {
  ...
  // DÉCONNEXION RÉELLE À DISTANCE VIA WEBSOCKET
  const handleDisconnect = (agentName: string) => {
    if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({
        ...
        to: agentName,
        content: { action: 'kill_agent', target_agent: agentName }
      }));
    }
                ...
                <tr key={i} className="hover:bg-zinc-800/30 transition">
                  ...
                  <td className="p-4 text-right">
                    <button 
                      ...
                      title="Disconnect Agent (Remote Kill)"
                    >
...
print("✅ COMPOSANT AGENTS DÉVOLU AU REMOTE KILL-SWITCH AVEC SUCCÈS !")
