...
layout_path = os.path.expanduser('~/nexus/portal/src/app/(app)/layout.tsx')
topbar_path = os.path.expanduser('~/nexus/portal/src/components/DashboardTopbar.tsx')
agents_path = os.path.expanduser('~/nexus/portal/src/app/(app)/agents/page.tsx')

# 1. Mise à jour de layout.tsx (Changement de l'icône Bot par Users/Cpu)
...
# 2. Mise à jour de DashboardTopbar.tsx (remplacement de Bot par Users)
if os.path.exists(topbar_path):
    with open(topbar_path, 'r', encoding='utf-8') as f:
        topbar_code = f.read()
    
    topbar_code = topbar_code.replace('Bot,', 'Users,')
    topbar_code = topbar_code.replace('icon: Bot', 'icon: Users')
    
    with open(topbar_path, 'w', encoding='utf-8') as f:
        f.write(topbar_code)

# 3. Mise à jour de agents/page.tsx (remplacement de Bot par Users)
if os.path.exists(agents_path):
    with open(agents_path, 'r', encoding='utf-8') as f:
        agents_code = f.read()
    
    agents_code = agents_code.replace('Bot,', 'Users,')
    agents_code = agents_code.replace('<Bot className="w-4 h-4 text-slate-400" />', '<Users className="w-4 h-4 text-slate-400 stroke-[1.5]" />')
    agents_code = agents_code.replace('<Bot className="w-4 h-4 text-white" />', '<Users className="w-4 h-4 text-white stroke-[1.5]" />')
    
    with open(agents_path, 'w', encoding='utf-8') as f:
        f.write(agents_code)

print("✅ TOUTES LES ICÔNES DE LA SIDEBAR ET DU MENU SONT DÉSORMAIS DES SVG VECTORIELS PURS (Users, Network, Key, ShieldCheck, CreditCard, Settings) !")
